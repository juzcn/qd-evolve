...
Concrete implementation details: architecture, module layout, class hierarchy, data flow, key patterns. For design philosophy and rationale, see [DESIGN.md](DESIGN.md).
...
Single `config.json` file. No env vars. Each agent gets its own provider, model, memory DB, server binding, and toolbox state. Global defaults as fallback. `provider: "human"` identifies human agents.
...
│   ├── group_chat_human.py  # GroupChatHuman — wraps MqttHumanAgent
