...
def create_agent_core(
    ...
    # ── Create A2AAgent (extends Agent with A2A identity + event subscribers) ──
    card = AgentCard(
        name=entry.name,
    ...
    a2a_agent._provider_name = entry.effective_provider(settings)
    a2a_agent._model = entry.effective_model(settings)
...
def create_agent(
    ...
    """Create a plain Agent (no A2A identity) from config.json agents list.

    Same as create_agent_core but returns Agent instead of A2AAgent.
    ...
    total_tools = len(registry.list_tools())
    logger.debug(
        "Loader [%s]: prompt %d tools (%d preload), %d unloaded skills, %d unloaded cli",
    ...
    # ── Disable A2A tools (single agent, no A2A) ─────────────────
    for tname in ("delegate_to", "send_task", "get_task", "cancel_task"):
        td = registry.get(tname)
        if td:
            td.enabled = False
    logger.info("Loader [%s]: A2A disabled (chat mode — single agent)", name)

    # ── Create Agent (no A2A identity) ────────────────────────────
    agent = Agent(
        settings=settings,
        registry=registry,
        providers=providers,
