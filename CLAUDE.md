...
- **Shared toolbox, per-agent config.** ToolRegistry is shared infrastructure — all tool definitions come from the same registry. But each agent has its own section in `toolbox.json` under `agents.<name>` for preload/enable/disable.
- **Agent config in config.json.** `agents_config` section with `chat_agent`, `agents` (list of AgentEntry), and `topology` (sub-section with default_mode, default_transport, transports, relations). `/agents` command switches at runtime and saves to config.json. No `--agent` CLI option or `serve` command — agent selection is runtime only.
- **Topology config.** `agents_config.topology` in config.json defines agent relationships (peer/master-worker) and transport mode per agent pair. Default peer-to-peer, configurable master-worker. Cross-machine agents auto-detected as http.
...
- **Per-agent memory db.** `AgentEntry.memory_db` in config.json specifies the SQLite file for each agent (default: `"memory.db"`). Full isolation — each agent has its own db.
